...
문제) 학번, 이름, 국어점수, 영어점수, 수학점수, 총점, 등수를 멤버로 갖는
	  ...
	  이 Student2객체들은 List에 저장하여 관리한다.
	  List에 저장된 데이터들을 학번의 오름차순으로 정렬하여 출력하는 부분과
	  총점의 역순으로 정렬하는 부분을 프로그램 하시오.
	  (총점이 같으면 학번의 내림차순으로 정렬되도록 한다.)
	  (학번 정렬기준은 Student2클래스 자체에서 제공하도록 하고,
	   총점 정렬기준은 외부클래스에서 제공하도록 한다.)
*/
public class T05_StudentTest_Answer {
	
	public static void main(String[] args) {
		T05_StudentTest_Answer stdTest = new T05_StudentTest_Answer();
		
		List<Student2> stdList = new ArrayList<Student2>();
		
		stdList.add(new Student2("201801", "홍길동", 90,95,80));
		stdList.add(new Student2("201803", "성춘향", 90,75,70));
		stdList.add(new Student2("201807", "강감찬", 95,95,80));
		stdList.add(new Student2("201805", "변학도", 80,95,90));
		stdList.add(new Student2("201802", "일지매", 100,85,80));
		stdList.add(new Student2("201804", "이순신", 60,65,60));
		stdList.add(new Student2("201806", "이몽룡", 90,100,90));
	...
	public void setRanking(List<Student2> stdList){
		for(Student2 std : stdList){  // 등수를 구할 자료(기준 자료)
			int rank = 1;
			for(Student2 std2 : stdList){ // 비교할 자료
...
class Student2 implements Comparable<Student2>{
	...
	public Student2(String num, String name, int kor, int eng, int mat) {
		super();
		this.num = num;
		this.name = name;
		this.kor = kor;
